...
int main(int argc, char **argv)
{
    ...
    hector_uav_msgs::EnableMotors eSrv;
    eSrv.request.enable = true;
    std::string uav_frame_id = "world"; 
    int uavId=999;
    n.getParam("uavId", uavId);
    if(uavId==1)
    {
      init_pos[1]+=1;
    ros::service::call("/uav1/enable_motors", eSrv);
    uav_frame_id = "uav1/"+uav_frame_id;

    }else if(uavId==2)
    {
      init_pos[1]-=1;
    ros::service::call("/uav2/enable_motors", eSrv);
    uav_frame_id = "uav2/"+uav_frame_id;

    }else
    {
      ros::service::call("/enable_motors", eSrv);
    }
    std::cout<<"uav "<<uavId<<"'s init pos is("<<init_pos[0]<<","<<init_pos[1]<<","<<init_pos[2]<<")"<<std::endl;
...
//    ros::Publisher ref_vel_pub = n.advertise<geometry_msgs::TwistStamped>("command/twist",1);
    ...
    while (ros::ok())
    {
        ...
        }else
        {
            ...
            cmdPos.header.stamp = ros::Time::now();
            cmdPos.header.frame_id = uav_frame_id;
